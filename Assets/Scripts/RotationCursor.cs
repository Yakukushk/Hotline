        ...
        if (isActiveRotation == false) // condition 
        {
            mousePos = cam.ScreenToWorldPoint(new Vector3(Input.mousePosition.x, Input.mousePosition.y,
                ...
                cam.transform.position
                    .z)); // valued parametrs for vector and developing input's for all axis (x,y,z) minus camera position z
            rig.transform.eulerAngles = new Vector3(0, 0,
                Mathf.Atan2((mousePos.y - transform.position.y), (mousePos.x - transform.position.x)) *
                Mathf.Rad2Deg); // arctg((mousePos for y minus position hero for y and same method for x))^2
